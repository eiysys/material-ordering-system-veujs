  ...
  <div id="content-div">
    <md-card style="height: -webkit-fill-available">
      <md-card-header>
        ...
        <router-link to="/"><img src="../../assets/persuit-logo.png" class="img img-responsive" width="250" height="70"></router-link>
        <!-- <a href="/#/"</a> -->
        <br>
        <div class="md-title text-center">PO Dashboard</div><br>
      </md-card-header>
      <md-card-actions>
                  ...
                  <template v-else>
                    <tr v-for="codeobj in item.fpo" bgcolor="#F7F7F7">
                      <template v-if="codeobj">
                        <template v-if="showCreateAndButton ">
                          <td v-if='codeobj.objID == ""'><input type="checkbox" v-bind:value="{SO: order._id, item:item.productType, fabric:codeobj.code, so_row:item.so_row}" v-model="FPO_Order"></td>
                          <td v-else>-</td>
                        </template>
                    ...
                    <template v-if="item.lpo != 11">
                      <tr bgcolor="#DFE3EE">
                        ...
                        <template v-if="showCreateAndButton">
                          ...
                          <td v-else><input type="checkbox" v-bind:value="{SO: order._id, item:item.productType, so_row:item.so_row}" v-model="LPO_Order"></td>
                        </template>
                        <template  v-else-if="!showCreateAndButton">
                          <td>-</td>
                        </template>
...
export default {
  ...
  methods: {
    ...
    createFPO: function () {
      ...
      document.cookie = 'fpo_order_data='+JSON.stringify(this.FPO_Order)+';expires='+date.toGMTString()+';path=/';
      Router.push('/fpo/new')
    ...
    createLPO: function () {
      ...
      // document.cookie = 'fpo_order="dab";expires='+date.toGMTString()+';path=/';
      document.cookie = 'lpo_order_data='+JSON.stringify(this.LPO_Order)+';expires='+date.toGMTString()+';path=/';
      Router.push('/lpo/new')
    ...
    getAllSalesData: function () {
      ...
      var salesURL = this.apiURL + 'salesOrder' + '/?token=' + this.authData.passwordHash + '&' + 'staffId=' + this.authData._id
      this.$http.get(salesURL).then(response => {
        this.salesOrdersIncomplete = response.body;
      }, response => {
    ...
    updateSalesDataFPO: async function () {
      var salesData = this.salesOrdersIncomplete;
      for(let i=0; i < salesData.length; i++ ) {
        for(let j=0; j < salesData[i].itemsDetail.length;j++ ) {
            if(salesData[i].itemsDetail[j].fpo){
                for(let k=0; k<salesData[i].itemsDetail[j].fpo.length; k++) {
                    if(salesData[i].itemsDetail[j].fpo[k].objID != "") {
                      let po_row = salesData[i].itemsDetail[j].fpo[k].po_row;
                      let so_row = salesData[i].itemsDetail[j].fpo[k].so_row;
                      // let url = this.apiURL + 'api/fabric-order/' + salesData[i].itemsDetail[j].fpo[k].objID;

                      var fpoURL = this.apiURL + 'api/fabric-order/' + salesData[i].itemsDetail[j].fpo[k].objID + '/?token=' + this.authData.passwordHash + '&' + 'staffId=' + this.authData._id;
                      await this.$http.get(fpoURL).then(response => {
                        for(let l=0;l<response.body.fab_data.length; l++) {
                          if (response.body.fab_data[l].po_row == po_row && response.body.fab_data[l].so_row == so_row) {
                            salesData[i].itemsDetail[j].fpo[k].unit = response.body.fab_data[l].unit;
                            salesData[i].itemsDetail[j].fpo[k].price = response.body.fab_data[l].price;
                            salesData[i].itemsDetail[j].fpo[k].quantity = response.body.fab_data[l].quantity;
                            salesData[i].itemsDetail[j].fpo[k].vendor = response.body.vendor;
                            salesData[i].itemsDetail[j].fpo[k].date = response.body.createdDate;
                            salesData[i].itemsDetail[j].fpo[k].arrived_date = response.body.fab_data[l].arrived_date;
                          }
                        }
      ...
      var st_obj = JSON.stringify(salesData);
      this.salesOrderWithoutLPO = JSON.parse(st_obj);
      this.updateSalesDataLPO();
    },
    updateSalesDataLPO: async function () {
      var salesData = this.salesOrderWithoutLPO;
      for(let i=0; i < salesData.length; i++ ) {
        // Items of sales order
        for (let j=0; j < salesData[i].itemsDetail.length; j++) {
          if (salesData[i].itemsDetail[j].lpo) {

            var lpoURL = this.apiURL + 'api/labor-order/' + salesData[i].itemsDetail[j].lpo.objID + '/?token=' + this.authData.passwordHash + '&' + 'staffId=' + this.authData._id;
            var salesID = salesData[i]._id;
            var sales_row = salesData[i].itemsDetail[j].so_row;

            // Fetch labor order
            await this.$http.get(lpoURL).then(response => {
              for(let k=0;k<response.body.lab_data.length;k++) {
                if (salesID == response.body.lab_data[k].SO && sales_row == response.body.lab_data[k].so_row) {
                    salesData[i].itemsDetail[j].lpo.lpo_row = response.body.lab_data[k].lpo_row;
                    salesData[i].itemsDetail[j].lpo.createdDate = response.body.createdDate;
                    salesData[i].itemsDetail[j].lpo.vendor = response.body.vendor;
                    salesData[i].itemsDetail[j].lpo.price = response.body.lab_data[k].price;
                    salesData[i].itemsDetail[j].lpo.quantity = response.body.lab_data[k].quantity;
                    salesData[i].itemsDetail[j].lpo.arrived_date = response.body.lab_data[k].arrived_date;
      ...
      var st_obj = JSON.stringify(salesData);
      this.salesOrderWithoutAPO = JSON.parse(st_obj);
      this.updateSalesDataAPO();
      // console.log(this.salesOrders);
    },
    updateSalesDataAPO: async function () {
      var salesData = this.salesOrderWithoutAPO;
      ...
      for(let i=0; i < salesData.length; i++ ) {
        ...
        for (let j=0; j < salesData[i].itemsDetail.length; j++) {
          ...
          try {
            if (salesData[i].itemsDetail[j].apo.length > 0) {

              for (let k=0; k<salesData[i].itemsDetail[j].apo.length ; k++ ) {
                // console.log('aaa');
                // console.log(salesData[i].itemsDetail[j].apo[k]);
                var apoURL = this.apiURL + 'api/alteration-order/' + salesData[i].itemsDetail[j].apo[k].objID + '/?token=' + this.authData.passwordHash + '&' + 'staffId=' + this.authData._id;
                var salesID = salesData[i]._id;
                var sales_row = salesData[i].itemsDetail[j].so_row;
                await this.$http.get(apoURL).then(response => {
                  // console.log('all APO');
                  // console.log(response);
                  salesData[i].itemsDetail[j].apo[k].createdDate = response.body.createdDate;
                  salesData[i].itemsDetail[j].apo[k].arrived_date = response.body.arrived_date;
                  if (response.body.alteration_price) {
                    salesData[i].itemsDetail[j].apo[k].alteration_price = response.body.alteration_price;
                  }
                }, response => {
                  console.log(response);
                });
      ...
      var st_obj = JSON.stringify(salesData);
      this.salesOrders = JSON.parse(st_obj);
      this.salesMainOrders = JSON.parse(st_obj);
      // this.salesOrders = this
      // this.updateSalesDataAPO();
      this.dtablefun();
    },
